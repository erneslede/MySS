# Problema 3 con tiempos y abandonos aleatorios
...
def tiempo_max_espera():
    return random.uniform(20, 60)  # espera máxima entre 10 y 30 segundos
...
while t_actual < t_max:
    ...
    # CORRECCIÓN 3: no procesar eventos fuera del horizonte
    ...
    if tipo_evento == "llegada":
        ...
        # CORRECCIÓN 4: servidor queda libre primero, luego se evalúa la cola
        estado_servidor = 0
        prox_fin_servicio = float('inf')
        ...
        for i, (llegada, abandono) in enumerate(clientes_en_cola):
            if abandono == prox_abandono:
                ...
                break
        resultados.append([round(t_actual,1), f"Abandono (hora prevista: {round(prox_abandono,1)})", len(clientes_en_cola), estado_servidor])
