...
TS_MIN,  TS_MAX  = 180, 300

# Ausencias — en segundos (independiente por puesto)
TTRAB_MIN, TTRAB_MAX = 120, 180
TDES_MIN,  TDES_MAX  =  30,  60
...
if CON_AUSENCIAS_SERVIDOR:
    prox_salida  = [HORA_INICIO + gen_ttrab() for _ in range(N_PUESTOS)]
...
while t_actual < T_MAX:
    ...
    # ── SALIDA DEL SERVIDOR ───────────────────────────────────────────────────
        ...
        if estado[p] == 1 and prox_fin[p] != INF:
            ...
            prox_fin[p] = INF
        estado[p]        = 0
        prox_regreso[p]  = t_actual + gen_tdes()
        prox_salida[p]   = INF
    ...
    # ── REGRESO DEL SERVIDOR ──────────────────────────────────────────────────
        ...
        p = reg_p
        presente[p]     = True
        prox_salida[p]  = t_actual + gen_ttrab()
        prox_regreso[p] = INF
